...
Timeline.OriginalEventPainter.prototype._onClickInstantEvent = function(icon, domEvt, evt) {
    ...
    var left = c.left + Math.ceil(icon.offsetWidth / 2);
    var top = c.top + Math.ceil(icon.offsetHeight / 2);
    //console.log("Instant: Left " + left + " Top: " + top);
    this._showBubble(
...
/*
    if ("pageX" in domEvt) {
	console.log("Using pageX");
        var x = domEvt.pageX;
        var y = domEvt.pageY;
    } else {
*/
	//console.log("Using page coords");
        var c = SimileAjax.DOM.getPageCoordinates(target);
        var x = domEvt.offsetX + c.left;
        var y = domEvt.offsetY + c.top;
    //}
    //console.log("Duration: Left: " + x + " Top: " + y);
